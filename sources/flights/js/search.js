...
if (true) {
    goog.require("Iata.Citys.En");
} else {
    goog.require("Iata.Citys.Es");
}

/** @define {string}  URL_SEARCH Whether logging should be enabled. */
//var URL_SEARCH = "http://www.travelcloud.com.co/site/search.ashx";//
var URL_SEARCH = "http://localhost:49815/site/search.ashx";
//var URL_SEARCH = "js/search.js";
...
flights.Search.prototype.enterDocument = function () {
    ...
    goog.dom.classlist.add(this.getElement(), goog.getCssName("technocloud"));
...
flights.Search.prototype.createCortinilla = function () {
    this.dialogo = goog.dom.htmlToDocumentFragment(flights.template.search.dialog());
...
flights.Search.prototype.onSearchClick_ = function (event) {
    ...
    if (this.txtMultiO1.value === this.txtMultiD1.value) {
        /** @desc Seleccione destinos diferentes */
        var MSG_UNNAMED_SELECCIONE_DESTINOS_DIFERENTES = goog.getMsg('Seleccione destinos diferentes');
        alert(MSG_UNNAMED_SELECCIONE_DESTINOS_DIFERENTES);
        return false;
    }
    ...
    if (!this.modal_vuelos_1.checked) {
        ...
        if (this.dp_txtFechaMultiO1.getDate().date > this.dp_txt2VFechaMulti.getDate().date) {
            /** @desc La fecha de regreso no puede ser menor */
            var MSG_UNNAMED_LA_FECHA_DE_REGRESO_NO_PUEDE_SER_MENOR = goog.getMsg('La fecha de regreso no puede ser menor');
            alert(MSG_UNNAMED_LA_FECHA_DE_REGRESO_NO_PUEDE_SER_MENOR);
            return false;
        }
    }
    ...
    /** @desc Adulto */
    var MSG_UNNAMED_ADULT = goog.getMsg('Adulto');
    /** @desc Ninos */
    var MSG_UNNAMED_NINOS = goog.getMsg('Ninos')
    /** @desc Infantes */
    var MSG_UNNAMED_INFANTS = goog.getMsg('Infantes')

    this.passenger = [
        { Type: "ADT", Desc: MSG_UNNAMED_ADULT, Cant: parseInt(this.ddlMultiAdults.value) },
        { Type: "CNN", Desc: MSG_UNNAMED_NINOS, Cant: parseInt(this.ddlMultiNinios.value) },
        { Type: "INF", Desc: MSG_UNNAMED_INFANTS, Cant: parseInt(this.ddlMultiBebes.value) }
    ];
...
flights.Search.prototype.hiddeAgesChildren = function () {
    try {
        ...
        var Selec = parseInt(document.getElementById(Campo).value);
        var ageChilds = goog.dom.getElementByClass(goog.getCssName("ageChilds"));
        ageChilds.innerHTML = "";

        for (var i = 0; i < Selec; i++) {
            var select = goog.dom.htmlToDocumentFragment(flights.template.search.renderAgeChilds());
            goog.dom.appendChild(ageChilds, select);
        }
...
flights.Search.prototype.hiddenAgesBabies = function () {
    try {
        ...
        var Campo = this.ids.ddlMultiInfants;
        var Selec = parseInt(document.getElementById(Campo).value);
        var ageInfants = goog.dom.getElementByClass(goog.getCssName("ageInfants"));
        ageInfants.innerHTML = "";

        for (var i = 0; i < Selec; i++) {
            var select = goog.dom.htmlToDocumentFragment(flights.template.search.renderAgeChilds());
            goog.dom.appendChild(ageInfants, select);
        }
...
flights.Search.prototype.searchflightsEventSource = function (strDataForm) {
    ...
    source.onmessage = goog.bind(function (event) {
        ...
        if (event.data === "") {
            ...
            /** @desc itente */
            var MSG_UNNAMED_INTENTE = goog.getMsg('No se encontraron resultados para la busqueda intente de nuevo');
            alert(MSG_UNNAMED_INTENTE);
            this.cortinilla.style.display = "none";
            return false;
        }
        if (event.data === "EOF") {
...
flights.Search.prototype.searchflights = function (strDataForm) {
    ...
    //jsonpSearch.setRequestTimeout(60000);
    //invocamos la url
    ...
    }, this), goog.bind(function (obj) {
        //si hay problemas de conexion
        //volvemos invisible la cortinilla
        this.cortinilla.style.display = "none";
        /** @desc MSG_UNNAMED_ERROR */
        var MSG_UNNAMED_ERROR = goog.getMsg('Error de conexion');
        alert(MSG_UNNAMED_ERROR);
...
flights.Search.prototype.setConfiguration = function () {
    ...
    //debugger;
    var widget_app_technocloud = goog.dom.getElement("widget_app_technocloud");
    var data_width = goog.dom.dataset.get(widget_app_technocloud, "width");
    var data_top = goog.dom.dataset.get(widget_app_technocloud, "top");
    var data_bg_color = goog.dom.dataset.get(widget_app_technocloud, "bgcolor");
    var data_text_color = goog.dom.dataset.get(widget_app_technocloud, "textcolor");
    var strStyle = "";

    /* COLORES */
    //background
    if (data_bg_color !== null) {
        strStyle += "." + goog.getCssName("background-color") +
                    ", ." + goog.getCssName("goog-date-picker") + "{    " +
               "background-color: " + data_bg_color + " !important;" +
               " }";
        //border
        strStyle += "." + goog.getCssName("border-color") +
                    ", ." + goog.getCssName("goog-date-picker") + "{" +
                "    border: 1px solid " + data_bg_color + " !important;" +
                "}";

        strStyle += "." + goog.getCssName("goog-date-picker") + "{" +
        "    background: " + data_bg_color + " !important;" +
        "}";
    }

    //text
    if (data_text_color !== null) {

        strStyle += "." + goog.getCssName("label-color") + "{" +
        "    color: " + data_text_color + " !important;" +
        "}";

        strStyle += "." + goog.getCssName("goog-date-picker") + " th {" +
        "    font-size: .9em;" +
        "    color: " + data_text_color + " !important;" +
        "}";

        strStyle += "." + goog.getCssName("goog-date-picker") + " button {" +
        "    color: " + data_text_color + " !important;" +
        "    font-size: 20px;" +
        "    text-transform: uppercase;" +
        "}";

        strStyle += "." + goog.getCssName("text-color") + "{    " +
        "    color: " + data_text_color + " !important;" +
        "}";
    }

    //dimentions
    if (data_width !== null && data_top !== null) {
        strStyle += "." + goog.getCssName("ux-common-main") + "{    " +
