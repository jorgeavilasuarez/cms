...
//goog.require("flights.styles.results");
...
flights.Results.prototype.renderFlightsResults = function () {
    ...
    /** @desc Parada */
    var MSG_UNNAMED_PARADA = goog.getMsg('Parada');
    /** @desc Directo */
    var MSG_UNNAMED_DIRECTO = goog.getMsg('Directo');
    ...
    for (var c = 0, max = tcAirSearchRSField.length; c < max; c++) {
        ...
        //traduccion paradas
        for (var k = 0; k < tcAirSearchRSField[c]["tcAirRouteField"].length; k++) {
            for (var i = 0; i < tcAirSearchRSField[c]["tcAirRouteField"][k]["tcAirItineraryField"].length; i++) {
                tcAirSearchRSField[c]["tcAirRouteField"][k]["tcAirItineraryField"][i]["stopNameField"] = tcAirSearchRSField[c]["tcAirRouteField"][k]["tcAirItineraryField"][i]["stopNameField"].replace(/Parada/g, MSG_UNNAMED_PARADA);
                tcAirSearchRSField[c]["tcAirRouteField"][k]["tcAirItineraryField"][i]["stopNameField"] = tcAirSearchRSField[c]["tcAirRouteField"][k]["tcAirItineraryField"][i]["stopNameField"].replace(/Directo/g, MSG_UNNAMED_DIRECTO);
            }
        }
...
flights.Results.prototype.showRegisterModule = function () {
    ...
    if (typeof this.availability !== "undefined" && this.availability != null && this.registerLoaded) {
        ...
        try {
            ...
            if (this.availability["DO_tcAirSearchRSJS"]["tcErrorField"]["codeField"] === "1" && !goog.DEBUG) {
...
flights.Results.prototype.setLabel_ = function (type, displayLabel, min, max) {
    ...
    /** @desc DESDE */
    var MSG_UNNAMED_DESDE = goog.getMsg('DESDE');
    /** @desc HASTA */
    var MSG_UNNAMED_HASTA = goog.getMsg('HASTA');

    displayLabel.innerHTML = MSG_UNNAMED_DESDE + ': ' + this.filterOptions_[type].values[min].label +
          '  ' + MSG_UNNAMED_HASTA + ': ' + this.filterOptions_[type].values[max].label;
};
    ...
    if (filterContainerElement == null) {
        ...
        goog.events.listen(btnFilterApply, goog.events.EventType.CLICK, goog.bind(function (event) {
            ...
            if (slider.getValue() !== slider.getMinimum() || maximum !== slider.getMaximum()) {
                ...
                this.setRemoveAllFilterActive_(goog.dom.dataset.get(event.currentTarget, "type"));
                //activamos el rango de filtros
                this.setFilterActiveRange_(goog.dom.dataset.get(event.currentTarget, "type"), slider.getValue(), (slider.getValue() + slider.getExtent()));
...
flights.Results.prototype.setSliderPriceUI_ = function () {
    /** @desc Precios */
    var MSG_UNNAMED_PRECIOS = goog.getMsg('Precios');

    var dataSlider = {
        type: "amountCurrencys",
        title: MSG_UNNAMED_PRECIOS,
...
flights.Results.prototype.setFilterAirLineUI_ = function () {
    ...
    /** @desc Aerolineas */
    var MSG_UNNAMED_AEROLINEAS = goog.getMsg('Aerolineas');

    var dataSlider = {
        type: "airLines",
        title: MSG_UNNAMED_AEROLINEAS,
...
flights.Results.prototype.onClickAllFilterCheckBoxUI_ = function (checksFilter, all_input, event) {
    ...
    //removemos el filtro
    this.setRemoveAllFilterActive_(goog.dom.dataset.get(all_input, "type"));
    //actualizamos la interfaz gráfica
    this.setUpdateUI_();
};
...
flights.Results.prototype.onClickFilterCheckBoxUI_ = function (checksFilter, all_input, event) {
    ...
    if (event.currentTarget.checked) {
        //debugger;
        this.setFilterActive_(goog.dom.dataset.get(event.currentTarget, "type"), event.currentTarget.value, event.currentTarget.value);
        //habilitamos el check de todas las opciones
        all_input.checked = false;
        all_input.disabled = false;
    } else {
        //removemos filtro
        this.removeFilterActive_(goog.dom.dataset.get(event.currentTarget, "type"), event.currentTarget.value, event.currentTarget.value);
...
flights.Results.prototype.setFilterScalesUI_ = function () {
    ...
    /** @desc Escalas */
    var MSG_UNNAMED_ESCALAS = goog.getMsg('Escalas');
    var dataSlider = {
        type: "stops",
        title: MSG_UNNAMED_ESCALAS,
...
flights.Results.prototype.setSliderArrivalTimeUI_ = function () {
    ...
    /** @desc Horario de llegada */
    var MSG_UNNAMED_HORARIO_DE_LEGADA = goog.getMsg('Horario de llegada');

    var dataSlider = {
        type: "arrivalTimes",
        title: MSG_UNNAMED_HORARIO_DE_LEGADA,
...
flights.Results.prototype.setSliderDepartureTimeUI_ = function () {
    ...
    /** @desc Horario de salida */
    var MSG_UNNAMED_HORARIO_DE_SALIDA = goog.getMsg('Horario de salida');

    var dataSlider = {
        type: "departureTimes",
        title: MSG_UNNAMED_HORARIO_DE_SALIDA,
...
flights.Results.prototype.setFilterDepartureAirportsUI_ = function () {
    ...
    /** @desc Aeropuertos de salida */
    var MSG_UNNAMED_AEROPUERTOS_DE_SALIDA = goog.getMsg('Aeropuertos de salida');


    var dataSlider = {
        type: "departureAirports",
        title: MSG_UNNAMED_AEROPUERTOS_DE_SALIDA,
...
flights.Results.prototype.setFilterArrivalAirportsUI_ = function () {
    ...
    /** @desc Aeropuertos de llegada */
    var MSG_UNNAMED_AEROPUERTOS_DE_LLEGADA = goog.getMsg('Aeropuertos de llegada');

    var dataSlider = {
        type: "arrivalAirPorts",
        title: MSG_UNNAMED_AEROPUERTOS_DE_LLEGADA,
...
flights.Results.prototype.setFilterMarketingCabinsUI_ = function () {
    ...
    /** @desc Clases */
    var MSG_UNNAMED_AEROPUERTOS_DE_CLASES = goog.getMsg('Clases');

    var dataSlider = {
        type: "marketingCabins",
        title: MSG_UNNAMED_AEROPUERTOS_DE_CLASES,
...
flights.Results.prototype.updateActiveFiltersUI_ = function () {
    ...
    for (var d = 0; d < btnRemoveFilters.length; d++) {
        this.getHandler().listen(btnRemoveFilters[d], goog.events.EventType.CLICK, function (event) {
            this.removeFilterActive_(goog.dom.dataset.get(event.currentTarget, "type"), goog.dom.dataset.get(event.currentTarget, "value"));
            //actualizamos la interfaz grafica
            this.updateActiveFiltersUI_();
        });
    }
...
flights.Results.prototype.getMetaData_ = function (obj) {
    ...
    for (var key in obj) {
        for (d = 0; d < CITYS.length; d++) {
            ...
            var index = goog.array.indexOf(this.metaData, obj[key]);
...
flights.Results.prototype.setAirport_ = function (obj) {
    ...
    if (goog.array.indexOf(this.metaData, obj[c]) !== -1) {
        return;
    }
...
flights.Results.prototype.getFilterOptions_ = function (objtcAirSearchRSField) {
    ...
    function getFilterValues(objFilter, id, idItinerary, value, label) {
        if (typeof objFilter[id] === "undefined") {
            ...
            if (goog.array.indexOf(objFilter[id].clusters, idItinerary) === -1) {
                objFilter[id].clusters.push(idItinerary);
            }
        }
    }
...
flights.Results.prototype.setCabinType_ = function (marketingCabinFields) {
    ...
    /** @desc Primera Premium */
    var MSG_UNNAMED_PRIMERA_PREMIUN = goog.getMsg('Primera Premium');
    /** @desc Primera */
    var MSG_UNNAMED_PRIMERA = goog.getMsg('Primera');
    /** @desc Negocios Premium */
    var MSG_UNNAMED_NEGOCIOS_PREMIUN = goog.getMsg('Negocios Premium');
    /** @desc Economica Premium */
    var MSG_UNNAMED_ECONOMICA_PREMIUN = goog.getMsg('Economica Premium');
    /** @desc Economica */
    var MSG_UNNAMED_ECONOMICA = goog.getMsg('Economica');


    for (var c = 0; c < marketingCabinFields.values.length; c++) {
        switch (marketingCabinFields.values[c].value) {
            case "P":
                marketingCabinFields.values[c].label = MSG_UNNAMED_PRIMERA_PREMIUN;
                break;
            case "F":
                marketingCabinFields.values[c].label = MSG_UNNAMED_PRIMERA;
                break;
            case "J":
                marketingCabinFields.values[c].label = MSG_UNNAMED_NEGOCIOS_PREMIUN;
                break;
            case "C":
                marketingCabinFields.values[c].label = MSG_UNNAMED_ECONOMICA_PREMIUN;
                break;
            case "Y":
                marketingCabinFields.values[c].label = MSG_UNNAMED_ECONOMICA;
                break;
    ...
    for (var k = 0; k < idsFilters.length; k++) {
        for (var n = 0; n < idsFilters[k].values.length; n++) {
            ...
            for (var m = 0; m < idsFilters.length; m++) {
                if (goog.array.indexOf(idsFilters[m].values, value) !== -1) {
                    founded++;
                }
            }
            if (founded >= idsFilters.length && goog.array.indexOf(idsFound, value) === -1) {
                idsFound.push(value);
            }
        }
    }
    ...
    for (var c = 0; c < idsFound.length; c++) {
        ...
        for (var keyFilter in this.filterOptions_) {
            for (var i = 0; i < this.filterOptions_[keyFilter].values.length; i++) {
                if (goog.array.indexOf(this.filterOptions_[keyFilter].values[i].clusters, idCluster) !== -1) {
