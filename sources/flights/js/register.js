...
//goog.require("flights.styles.register");
...
var URL_PAY = "http://payment.travelcloud.com.co/site/gateway.aspx";
...
flights.Register.prototype.enterDocument = function () {
    ...
    this.passengerInfoDiv.innerHTML = flights.template.register.passenger({ passenger: this.passengerInfo });
...
flights.Register.prototype.onReserveClick = function () {
    ...
    if (!this.chkAgree.checked) {
        /** @desc Debe aceptar los terminos y condiciones */
        var MSG_UNNAMED_TERMINOS_CONDICIONES = goog.getMsg('Debe aceptar los terminos y condiciones');
        alert(MSG_UNNAMED_TERMINOS_CONDICIONES);
        return false;
    }
    /*send data*/
    if (true) {
        ...
        goog.net.XhrIo.send(URL_BOOKING, goog.bind(function (obj) {
            ...
            try {
                ...
                if (this.booking["DO_tcAirSearchRSJS"]["tcErrorField"]["codeField"] === "1") {
                    ...
                    /** @desc Su reserva fue confirmada con el record */
                    var MSG_UNNAMED_SU_RESERVA_FUE_CONFIRMADA_CON_EL_RECORD = goog.getMsg('Su reserva fue confirmada con el record');

                    title.innerHTML = MSG_UNNAMED_SU_RESERVA_FUE_CONFIRMADA_CON_EL_RECORD + ": " + this.booking["DO_tcAirSearchRSJS"]["tcRecordIdField"]["pNRField"];
        ...
        goog.net.XhrIo.send(URL_CANCEL, goog.bind(function (obj) {
            ...
            try {
                ...
                if (this.booking["DO_tcAirSearchRSJS"]["tcErrorField"]["codeField"] === "1") {
                    ...
                    var MSG_UNNAMED_RESERVA_CANCELADA = goog.getMsg('Reserva cancelada');
                    alert(MSG_UNNAMED_RESERVA_CANCELADA);
                    this.objResults.getElement().style.display = "block";
                    this.dispose();
                }
            } catch (e) {
